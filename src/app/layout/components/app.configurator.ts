                    ...
                    <div class="pt-2 flex gap-2 flex-wrap">
                        @for (surface of surfaces; track surface.name) {
                            <button
                                ...
                                [ngClass]="{
                                    'outline outline-primary': selectedSurface() ? selectedSurface() === surface.name : isDarkTheme() ? surface.name === 'zinc' : surface.name === 'slate'
                                }"
                                [style]="{
                        ...
                        <ng-template #menu_theme_selector>
                            <div class="pt-2 flex gap-2 flex-wrap">
                                @for (theme of menuThemes; track theme) {
                                    <button
                                        type="button"
                                        [title]="selectedMenuTheme()"
                                        (click)="changeMenuTheme(theme.name)"
                                        [ngClass]="{'outline outline-primary': selectedMenuTheme() === theme.name}"
                        ...
                        <div class="pt-2 flex gap-2 flex-wrap">
                            @for (theme of topbarThemes; track theme) {
                                <button
                                    ...
                                    [ngClass]="{'outline outline-primary': selectedTopbarTheme() === theme.name}"
                                    class="cursor-pointer w-6 h-6 rounded-full flex shrink-0 items-center justify-center outline-offset-1 shado"
                                    [style]="{'background-color': theme.color}"
                                >
                                </button>
...
export class AppConfigurator {
    ...
    selectedTopbarTheme = computed(() => this.layoutService.layoutConfig().topbarTheme);

    selectedMenuTheme = computed(() => this.layoutService.layoutConfig().menuTheme);
