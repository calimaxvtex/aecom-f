                    ...
                    <div class="mb-4 flex items-center justify-between">
                        ...
                        <p-buttonGroup>
                            ...
                            <p-button *ngIf="selectedItems.length > 0" icon="pi pi-plus"
                                pTooltip="Buscar información de {{ selectedItems.length }} item(s) seleccionado(s)"
                                tooltipPosition="top" (onClick)="agregarSeleccionados()" styleClass="p-button-info p-button-raised">
                            </p-button>
                        </p-buttonGroup>
                    </div>
...
export class ItemsTestComponent implements OnInit {
    ...
    private cargarMarcas() {
        // Verificar si ya hay cache disponible
        if (this.marcasService.isCacheLoaded()) {
            console.log('✅ Usando cache de marcas existente');
    ...
    private cargarSubcategoriasCacheInicial() {
        console.log('📥 Cargando cache inicial de subcategorías...');
        this.subcategoriasService.loadAllSubcategorias().subscribe({
            next: (subcategorias) => {
                console.log(`✅ Cache de subcategorías cargado: ${subcategorias.length} registros`);
                this.messageService.add({
                    severity: 'success',
                    summary: 'Cache Cargado',
                    detail: `Subcategorías cacheadas: ${subcategorias.length} registros`,
                    life: 3000
                });
            },
            error: (error) => {
                console.error('❌ Error cargando cache de subcategorías:', error);
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error en Cache',
                    detail: 'No se pudo cargar el cache de subcategorías',
                    life: 4000
                });
            }
        });
    ...
    agregarSeleccionados() {
        ...
        // Preparar consulta al API con todos los items seleccionados
        const apiParams = {
        ...
        this.itemsService.getItems(apiParams).subscribe({
            next: (response) => {
                ...
                console.log(`✅ Consulta completada - ${response.data?.length || 0} registros recibidos`);
            },
            error: (error) => {
                console.error('❌ Error en la consulta al API:', error);
